...
class AdminService {
  ...
  Future<Map<String, dynamic>> dashboard() async {
    final response = await http.get(Uri.parse(ApiConfig.adminDashboard), headers: _headers);
    return _decodeMap(response);
  }

  Future<List<User>> employees() async {
    final response = await http.get(Uri.parse(ApiConfig.adminEmployees), headers: _headers);
  ...
  Future<Map<String, dynamic>> uploadLeads(String filePath) async {
    final request = http.MultipartRequest('POST', Uri.parse(ApiConfig.adminUploadLeads));
  ...
  Future<String> downloadReport({DateTime? startDate, DateTime? endDate}) async {
    final query = <String, String>{};
    if (startDate != null) query['startDate'] = startDate.toIso8601String();
    if (endDate != null) query['endDate'] = endDate.toIso8601String();

    final uri = Uri.parse(ApiConfig.adminReportDownload).replace(queryParameters: query);
    final response = await http.get(uri, headers: _headers);

    if (response.statusCode < 200 || response.statusCode >= 300) {
      final message = response.body.isEmpty ? 'Report download failed' : response.body;
